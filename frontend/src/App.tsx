...
export default function App() {
  ...
  const features = useMemo(
    () => (region ? all.filter((f) => f.properties.region === region) : all),
    [all, region]
  );
  ...
  const rail = (i: number) =>
    reduce
      ? {}
      : { initial: { opacity: 0, x: -16 }, animate: { opacity: 1, x: 0 }, transition: { duration: 0.5, delay: i * 0.08, ease: EASE } };

  return (
    <div className="stage">
      <MapView features={features} selected={selected?.properties.ocid ?? null} onSelect={setSelected} />
